...
function createAuthorizationTuple(
    chainId: number,
    address: string,
    nonce: number,
    privateKey: string
...
describeWithFrontier("Frontier RPC (EIP-7702 Set Code Authorization)", (context: any) => {
    ...
    step("should handle EIP-7702 transaction type 4 structure", async function () {
        ...
        const authorization = createAuthorizationTuple(
            CHAIN_ID,
            contractAddress,
            0,
            GENESIS_ACCOUNT_PRIVATE_KEY
    ...
    step("should handle multiple authorizations", async function () {
        ...
        const auth1 = createAuthorizationTuple(
            CHAIN_ID,
            contractAddress,
            0,
            GENESIS_ACCOUNT_PRIVATE_KEY
        ...
        try {
            ...
            if (receipt.status === "0x1") {
                ...
                if (delegationCheck.isDelegation) {
                    console.log(`Delegation set! Account ${newAccount.address} delegates to ${delegationCheck.address}`);

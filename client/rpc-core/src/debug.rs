...
pub trait DebugApi {
	...
	async fn raw_transaction(&self, hash: H256) -> Result<Option<Bytes>>;

	/// Returns an array of EIP-2718 binary-encoded receipts with the given number of hash.
	#[method(name = "debug_getRawReceipts")]
	async fn raw_receipts(&self, number: BlockNumber) -> Result<Vec<Bytes>>;

	/// Returns an array of recent bad blocks that the client has seen on the network.
	#[method(name = "debug_getBadBlocks")]
	fn bad_blocks(&self, number: BlockNumber) -> Result<Vec<()>>;
}

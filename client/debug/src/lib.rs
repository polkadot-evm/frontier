...
pub use fc_debug_core::{Debug as DebugT, DebugServer, TraceParams};
...
use fp_debug::{DebugRuntimeApi, TracerInput};
			...
			if let Some(transaction) = transactions.get(index) {
				...
				return match trace_type {
					single::TraceType::Raw {
						...
						proxy.using(f)?;
						Ok(Response::Single(
							fc_evm_tracing::formatters::Raw::format(proxy).ok_or(
								internal_err(

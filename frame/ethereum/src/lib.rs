...
impl<T: Config> Pallet<T> {
	...
	pub fn migrate_block_v2_to_v3() -> Weight {
		let db_weights = T::DbWeight::get();
		let mut weight: Weight = db_weights.reads(1);
		let item = b"CurrentBlock";
		...
		if let Some(block_v2) = block_v2 {
			weight = weight.saturating_add(db_weights.writes(1));
			let block_v3: ethereum::BlockV3 = block_v2.into();
	...
	#[cfg(feature = "try-runtime")]
	pub fn pre_migrate_block_v3() -> Result<Vec<u8>, &'static str> {
		let item = b"CurrentBlock";
		let block_v2 = frame_support::storage::migration::get_storage_value::<ethereum::BlockV2>(
			Self::name().as_bytes(),
			item,
		...
		if let Some(block_v2) = block_v2 {
			Ok((
				block_v2.header.number,
				block_v2.header.parent_hash,
				block_v2.transactions.len() as u64,
			)
				.encode())
		} else {
			Ok(Vec::new())
	...
	#[cfg(feature = "try-runtime")]
	pub fn post_migrate_block_v3(v2_data: Vec<u8>) -> Result<(), &'static str> {
		let (v2_number, v2_parent_hash, v2_transaction_len): (U256, H256, u64) = Decode::decode(
			&mut v2_data.as_slice(),
		)
		.expect("the state parameter should be something that was generated by pre_upgrade");
		let item = b"CurrentBlock";
		let block_v3 = frame_support::storage::migration::get_storage_value::<ethereum::BlockV3>(
			Self::name().as_bytes(),
			item,
			&[],
		);

		assert!(block_v3.is_some());

		let block_v3 = block_v3.unwrap();
		assert_eq!(block_v3.header.number, v2_number);
		assert_eq!(block_v3.header.parent_hash, v2_parent_hash);
		assert_eq!(block_v3.transactions.len() as u64, v2_transaction_len);
		Ok(())
	}
}

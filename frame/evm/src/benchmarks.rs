...
use crate::{Config, Pallet, EnsureAddressNever, EnsureAddressSame, EnsureAddressRoot,
	FeeCalculator, HashedAddressMapping, Event, BalanceOf, AddressMapping};
...
pub struct MockedAddressMapping;

impl AddressMapping<AccountId32> for MockedAddressMapping {
...
impl Config for Test {
	...
	type AddressMapping = MockedAddressMapping;
...
benchmarks! {
	test {
		...
		let caller: AccountId32 = create_funded_user::<Test>("caller", 0, 1000000000000000000u64.into());
		// let bytes: [u8; 20] = AsRef::<[u8; 32]>::as_ref(&caller)[0..20];
		let caller_address = H160::from_slice(&AsRef::<[u8; 32]>::as_ref(&caller)[0..20]); // WTF
		// let caller_address = H160::from_slice(caller.as_ref()[0..20]);

		// let caller_address = H160::from_slice(caller.as_ref()[0..20]);
		// let caller_address = H160::from(AsRef::<[u8; 32]>::as_ref(&caller)[0..20]); // WTF
		// let caller_address = H160::from(caller.as_byte_slice()[0..20]);
		/*
		let bytes: [u8] = caller.as_slice_of::<u8>().expect();
		let caller = H160::from(bytes[0..20]);

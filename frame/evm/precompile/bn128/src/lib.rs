...
fn read_fr(input: &[u8], start_inx: usize) -> Result<bn::Fr, PrecompileFailure> {
	if input.len() < start_inx + 32 {
		return Err(PrecompileFailure::Error { exit_status: ExitError::Other("Input not long enough".into())});
	}

	bn::Fr::from_slice(&input[start_inx..(start_inx + 32)])
		.map_err(|_| PrecompileFailure::Error { exit_status: ExitError::Other("Invalid field element".into())})
}
...
fn read_point(input: &[u8], start_inx: usize) -> Result<bn::G1, PrecompileFailure> {
	...
	let px = Fq::from_slice(&input[start_inx..(start_inx + 32)])
		.map_err(|_| PrecompileFailure::Error { exit_status: ExitError::Other("Invalid point x coordinate".into())})?;
	let py = Fq::from_slice(&input[(start_inx + 32)..(start_inx + 64)])
		.map_err(|_| PrecompileFailure::Error { exit_status: ExitError::Other("Invalid point y coordinate".into())})?;
	Ok(if px == Fq::zero() && py == Fq::zero() {
		G1::zero()
	} else {
		AffineG1::new(px, py)
			.map_err(|_| PrecompileFailure::Error { exit_status: ExitError::Other("Invalid curve point".into())})?
			.into()
	})
}
	...
	fn execute(
		...
		if let Some(sum) = AffineG1::from_jacobian(p1 + p2) {
			...
			sum.x().to_big_endian(&mut buf[0..32]).map_err(|_| {
				PrecompileFailure::Error { exit_status: ExitError::Other("Cannot fail since 0..32 is 32-byte length".into())}
			})?;
			sum.y().to_big_endian(&mut buf[32..64]).map_err(|_| {
				PrecompileFailure::Error { exit_status: ExitError::Other("Cannot fail since 32..64 is 32-byte length".into())}
			})?;
		}
		...
		let (ret_val, gas_cost) = if input.is_empty() {
			...
			for idx in 0..elements {
				let a_x = Fq::from_slice(&input[idx * 192..idx * 192 + 32])
					.map_err(|_| PrecompileFailure::Error { exit_status: ExitError::Other("Invalid a argument x coordinate".into())})?;

				let a_y = Fq::from_slice(&input[idx * 192 + 32..idx * 192 + 64])
					.map_err(|_| PrecompileFailure::Error { exit_status: ExitError::Other("Invalid a argument y coordinate".into())})?;

				let b_a_y =
					Fq::from_slice(&input[idx * 192 + 64..idx * 192 + 96]).map_err(|_| {
						PrecompileFailure::Error { exit_status: ExitError::Other("Invalid b argument imaginary coeff x coordinate".into())}
					})?;

				let b_a_x =
					Fq::from_slice(&input[idx * 192 + 96..idx * 192 + 128]).map_err(|_| {
						PrecompileFailure::Error { exit_status: ExitError::Other("Invalid b argument imaginary coeff y coordinate".into())}
					})?;

				let b_b_y =
					Fq::from_slice(&input[idx * 192 + 128..idx * 192 + 160]).map_err(|_| {
						PrecompileFailure::Error { exit_status: ExitError::Other("Invalid b argument real coeff x coordinate".into())}
					})?;

				let b_b_x =
					Fq::from_slice(&input[idx * 192 + 160..idx * 192 + 192]).map_err(|_| {
						PrecompileFailure::Error { exit_status: ExitError::Other("Invalid b argument real coeff y coordinate".into())}
					})?;
				...
				let b = if b_a.is_zero() && b_b.is_zero() {
					...
					G2::from(AffineG2::new(b_a, b_b).map_err(|_| {
						PrecompileFailure::Error { exit_status: ExitError::Other("Invalid b argument - not on curve".into())}
					})?)
				};
				let a = if a_x.is_zero() && a_y.is_zero() {
					...
					G1::from(AffineG1::new(a_x, a_y).map_err(|_| {
						PrecompileFailure::Error { exit_status: ExitError::Other("Invalid a argument - not on curve".into())}
					})?)
				};
				vals.push((a, b));
			}
